...
const RenderedTitle = (): ReactElement => {
  return (
    <h1 className='text-4xl text-white font-semibold pt-2 underline'>
      Same Series
    </h1>
  )
}
...
export default function SameSeries({ games }: Props) {
  ...
  return (
    <div id="same_series" className='text-center pt-12' style={{ width: '27%' }}>
      <RenderedTitle />
      <div id="same_series_inner">
        {
          games.results.length > 5 ? games.results.splice(0, 5).map(game => (
