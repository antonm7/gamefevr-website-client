        ...
        <div className="relative bg-gray-400 rounded-lg  overflow-hidden h-96" style={{height:'40rem'}}>
            ...
            <div className="flex-grow p-4">
                ...
                <div className="flex flex-row justify-between pt-6">
                    <div className="flex flex-row flex-nowrap">{game.genres.slice(0, 3).map((genre:ElementDescription,index:number) => <h2 key={index} className='pr-1 text-sm' style={{color:'#919191'}}>{genre.name}{index === game.genres.length - 1 || index === 2 ? '' : ','}</h2>)}</div>
                    <h2 className='pr-1 text-sm' style={{color:'#919191'}}>{game.released.slice(0,4)}</h2>
                </div>
            </div>
        </div>
    )
